...
[NOTE]
Az összes típusok halmaza stem:[cc "T"].
Az összes beépített cache típusok halmaza stem:[cc "C"].
...
. adott stem:[C in cc "C"], a hozzá tartozó bemenet literáljával konstruálható
. adott stem:[C in cc "C"], a hozzá tartozó bemenet std::move-olt változójával konstruálható
. adott stem:[C in cc "C"], a `get_value_ptr` tagfüggvény konstans
. adott stem:[C in cc "C"], a `get_value_ptr` tagfüggvény visszatérési típusa konstansra mutató
. adott stem:[C in cc "C"] ami literállal lett konstruálva, a `get_value_ptr` értéke nem nullptr
. adott stem:[C in cc "C"] ami std::move-al lett konstruálva, a `get_value_ptr` értéke nem nullptr
. adott stem:[C in cc "C"] ami literállal lett konstruálva, `get_value_ptr` nem dob kivételt
. adott stem:[C in cc "C"] ami std::move-al lett konstruálva, `get_value_ptr` nem dob kivételt
. adott stem:[C in cc "C"] ami std::move-al lett konstruálva, a `get_value_ptr` visszaadott pointer nem egyezik meg a bemenetre mutató pointer értékével

=== type_id

. adott stem:[T in cc "T"], type_id nemnulla értéked ad
. adott stem:[T,U in cc "T" (T != U)], type_id különböző értékeket ad
. adott stem:[T in cc "T"], type_id kétszer ugyan azt az értéked adja
. adott stem:[T in cc "T"], type_id == tagfüggvénye konstans
. adott stem:[T in cc "T"], type_id =! tagfüggvénye konstans
. adott stem:[T in cc "T"], type_id == tagfüggvénye nem dobhat
. adott stem:[T in cc "T"], type_id =! tagfüggvénye nem dobhat
