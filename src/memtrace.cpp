...
static const char* pretty[] = {"malloc(", "calloc(", "realloc(", "free(", "new", "delete", "new[]", "delete[]"};
...
static void*
canary_malloc(size_t size, unsigned char data) {
...
static call_t
pack(int f, const char* par_txt, int line, const char* file) {
    call_t ret;
    ret.f = f;
    ret.line = line;
...
static void
print_call(const char* msg, call_t call) {
...
static void
dump_memory(void const* mem, size_t size, size_t can_len, FILE* fp) {
...
void
mem_dump(void const* mem, size_t size, FILE* fp) {
...
static void
die(const char* msg, void* p, size_t size, call_t* a, call_t* d) {
...
static void
print_registry_item(registry_item* p) {
    if (p) {
...
int
allocated_blocks() { return allocated_blks; }

static BOOL
register_memory(void* p, size_t size, call_t call) {
    initialize();
    allocated_blks++;
#  ifdef MEMTRACE_TO_FILE
...
#  ifdef MEMTRACE_TO_MEMORY
static registry_item*
find_registry_item(void* p) {
...
static void
unregister_memory(void* p, call_t call) {
    initialize();
#  ifdef MEMTRACE_TO_FILE
...
START_NAMESPACE
void*
traced_malloc(size_t size, const char* par_txt, int line, const char* file) {
    void* p;
    initialize();
...
void*
traced_calloc(size_t count, size_t size, const char* par_txt, int line, const char* file) {
    void* p;
    initialize();
    size *= count;
...
void
traced_free(void* pu, const char* par_txt, int line, const char* file) {
    initialize();
    if (pu) {
...
void*
traced_realloc(void* old, size_t size, const char* par_txt, int line, const char* file) {
    void* p;
    size_t oldsize = 0;
    registry_item* n;
    initialize();
...
void
_set_new_handler(std::new_handler h) {
    initialize();
    _new_handler = h;
}
...
void
set_delete_call(int line, const char* file) {
    initialize();
...
void*
traced_new(size_t size, int line, const char* file, int func) {
    initialize();
    for (;;) {
...
void
traced_delete(void* pu, int func) {
    initialize();
    if (pu) {
...
void*
operator new(size_t size, int line, const char* file) THROW_BADALLOC {
...
void*
operator new(size_t size) THROW_BADALLOC {
...
void
operator delete(void* p) THROW_NOTHING {
...
void
operator delete(void* p, size_t) THROW_NOTHING {
    memtrace::traced_delete(p, FDELETE);
}
...
void
operator delete(void* p, int, const char*) THROW_NOTHING {
    memtrace::traced_delete(p, FDELETE);
}
