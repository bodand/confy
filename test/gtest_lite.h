...
/// Kivételt várunk
#define EXPECT_THROW(statement, exception_type) try { gtest_lite::test.tmp = false; statement; } catch (exception_type) { gtest_lite::test.tmp = true; } catch (...) { } \
    EXPECTTHROW(statement, "kivetelt dob.", "nem dobott '" #exception_type "' kivetelt.")

/// Kivételt várunk
#define EXPECT_ANY_THROW(statement) try { gtest_lite::test.tmp = false; statement; } catch (...) { gtest_lite::test.tmp = true; } \
    EXPECTTHROW(statement, "kivetelt dob.", "nem dobott kivetelt.")

/// Nem várunk kivételt
#define EXPECT_NO_THROW(statement) try { gtest_lite::test.tmp = true; statement; } catch (...) { gtest_lite::test.tmp = false; }\
    EXPECTTHROW(statement, "nem dob kivetelt.", "kivetelt dobott.")

/// Nem várunk kivételt
#define ASSERT_NO_THROW(statement) try { gtest_lite::test.tmp = true; statement; } catch (...) { gtest_lite::test.tmp = false; }\
    ASSERTTHROW(statement, "nem dob kivetelt.", "kivetelt dobott.")

/// Kivételt várunk és továbbdobjuk -- ilyen nincs a gtest-ben
#define EXPECT_THROW_THROW(statement, exception_type) try { gtest_lite::test.tmp = false; statement; } catch (exception_type) { gtest_lite::test.tmp = true; throw; } \
    EXPECTTHROW(statement, "kivetelt dob.", "nem dobott '" #exception_type "' kivetelt.")
...
namespace gtest_lite {
    ...
    public:
        ...
        bool
        astatus() { return status; }

        /// Eredményt adminisztráló tagfüggvény True a jó eset.
        std::ostream&
        expect(bool st, const char* file, int line, const char* expr, bool pr = false) {
            if (!st) {
                ++failed;
                status = false;
            }
    ...
    std::ostream&
    EXPECT_(T1 exp, T2 act, bool (*pred)(T1, T2), const char* file, int line, const char* expr, const char* lhs = "elvart", const char* rhs = "aktual") {
    ...
    std::ostream&
    EXPECT_(T1* exp, std::nullptr_t act, bool (*pred)(T1*, std::nullptr_t), const char* file, int line, const char* expr, const char* lhs = "elvart", const char* rhs = "aktual") {
        return test.expect(pred(exp, act), file, line, expr)
               << "** " << lhs << ": " << (void*) exp
               << "\n** " << rhs << ": " << (void*) act << std::endl;
    }
    ...
    inline std::ostream&
    EXPECTSTR(const char* exp, const char* act, bool (*pred)(const char*, const char*), const char* file, int line, const char* expr, const char* lhs = "elvart", const char* rhs = "aktual") {
        return test.expect(pred(exp, act), file, line, expr)
    ...
    template<typename E, typename S>
    std::ostream&
    EXPECTREGEXP(E exp, S str, int match, const char* err, const char* file, int line, const char* expr, const char* lhs = "regexp", const char* rhs = "string", const char* m = "elvart/illeszkedik") {
    ...
    inline bool
    eqstrcase(const char* a, const char* b) {
        if (a != NULL && b != NULL) {
    ...
    template<typename T1, typename T2>
    bool
    ne(T1 a, T2 b) { return a != b; }

    inline bool
    nestr(const char* a, const char* b) {
        if (a != NULL && b != NULL)
            return strcmp(a, b) != 0;
        return false;
    }
    ...
    template<typename T1, typename T2>
    bool
    le(T1 a, T2 b) { return a <= b; }

    template<typename T1, typename T2>
    bool
    lt(T1 a, T2 b) { return a < b; }

    template<typename T1, typename T2>
    bool
    ge(T1 a, T2 b) { return a >= b; }

    template<typename T1, typename T2>
    bool
    gt(T1 a, T2 b) { return a > b; }
